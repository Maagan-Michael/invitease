...
def get_user_names(users: KeycloakUserRepository = Depends(
        create_users_repository), user_ids: Optional[List[str]] = Query([])):
...
@router.post("/change_admitted/{invitation_id}",
             summary="Updates the invitation information.")
def change_admitted(
    request: UpdateInviteesRequest,
